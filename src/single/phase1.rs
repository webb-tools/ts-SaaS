...
use crate::single::batched_pairing::{BatchedPairingChecker11, BatchedPairingChecker12};
use crate::single::dlog;
use crate::single::log::{ContributionHash, Hashable, Phase};

use super::group::GroupHasher;
...
pub struct RawCRSElements<E: Pairing> {
    ...
    pub x_1: Vec<E::G1>,
    pub x_2: Vec<E::G2>,
    pub alpha_x_1: Vec<E::G1>,
    pub beta_x_1: Vec<E::G1>,
}
...
impl<E: Pairing> RawCRSElements<E> {
    ...
    pub fn validate(self) -> anyhow::Result<CRSElements<E>> {
        ...
        let mut checker0 =
            BatchedPairingChecker12::<E>::new(E::G2::generator(), E::G1::generator());
        ...
        let mut checker1 = BatchedPairingChecker12::<E>::new(E::G2::generator(), self.alpha_1);
        ...
        let mut checker2 = BatchedPairingChecker12::<E>::new(E::G2::generator(), self.beta_1);
        ...
        let mut checker3 = BatchedPairingChecker11::<E>::new(self.x_2[1], E::G2::generator());
...
impl<E: Pairing> Hashable for RawCRSElements<E> {
    fn hash(&self) -> ContributionHash {
        let mut hasher = GroupHasher::<E>::new(b"PC$:crs_elmnts1");
...
impl<E: Pairing> CRSElements<E> {
    ...
    pub fn root(d: usize) -> Self {
        ...
        let raw = RawCRSElements {
            alpha_1: E::G1::generator(),
            beta_1: E::G1::generator(),
            beta_2: E::G2::generator(),
            x_1: vec![E::G1::generator(); (2 * d - 2) + 1],
            x_2: vec![E::G2::generator(); (d - 1) + 1],
            alpha_x_1: vec![E::G1::generator(); (d - 1) + 1],
            beta_x_1: vec![E::G1::generator(); (d - 1) + 1],
        };
        Self { degree: d, raw }
    }
}
...
pub(crate) struct LinkingProof<E: Pairing> {
    alpha_proof: dlog::Proof<E>,
    beta_proof: dlog::Proof<E>,
    x_proof: dlog::Proof<E>,
}
...
impl<E: Pairing> Hashable for RawContribution<E> {
    fn hash(&self) -> ContributionHash {
        let mut hasher = GroupHasher::<E>::new(b"PC$:contrbution1");
...
mod test {
    ...
    fn make_crs<E: Pairing>(
        ...
        RawCRSElements {
            alpha_1: E::G1::generator() * alpha,
            beta_1: E::G1::generator() * beta,
            beta_2: E::G2::generator() * beta,
            x_1: vec![
                E::G1::generator(),
                E::G1::generator() * x,
                E::G1::generator() * (x * x),
            ],
            x_2: vec![E::G2::generator(), E::G2::generator() * x],
            alpha_x_1: vec![E::G1::generator() * alpha, E::G1::generator() * (alpha * x)],
            beta_x_1: vec![E::G1::generator() * beta, E::G1::generator() * (beta * x)],
        }
    }

    fn non_trivial_crs<E: Pairing>() -> RawCRSElements<E> {
        let alpha = E::ScalarField::rand(&mut OsRng);
        let beta = E::ScalarField::rand(&mut OsRng);
        let x = E::ScalarField::rand(&mut OsRng);

        make_crs::<E>(alpha, beta, x)
    }

    #[test]
    fn test_root_crs_is_valid() {
        let root = CRSElements::<Bn254>::root(D);
        assert!(root.raw.validate().is_ok());
    }

    #[test]
    fn test_nontrivial_crs_is_valid() {
        let crs = non_trivial_crs::<Bn254>();
        assert!(crs.validate().is_ok());
    }

    #[test]
    fn test_changing_alpha_makes_crs_invalid() {
        let mut crs = non_trivial_crs::<Bn254>();
        crs.alpha_1 = <Bn254 as Pairing>::G1::generator();
    ...
    fn test_changing_beta_makes_crs_invalid() {
        let mut crs = non_trivial_crs::<Bn254>();
        crs.beta_1 = <Bn254 as Pairing>::G1::generator();
        assert!(crs.validate().is_err());
    }

    #[test]
    fn test_setting_zero_elements_makes_crs_invalid() {
        let alpha = <Bn254 as Pairing>::ScalarField::rand(&mut OsRng);
        let beta = <Bn254 as Pairing>::ScalarField::rand(&mut OsRng);
        let x = <Bn254 as Pairing>::ScalarField::rand(&mut OsRng);

        let crs0 = make_crs::<Bn254>(<Bn254 as Pairing>::ScalarField::zero(), beta, x);
        assert!(crs0.validate().is_err());
        let crs1 = make_crs::<Bn254>(alpha, <Bn254 as Pairing>::ScalarField::zero(), x);
        assert!(crs1.validate().is_err());
        let crs2 = make_crs::<Bn254>(alpha, beta, <Bn254 as Pairing>::ScalarField::zero());
    ...
    fn test_bad_powers_of_x_makes_crs_invalid() {
        ...
        let beta = <Bn254 as Pairing>::ScalarField::rand(&mut OsRng);
        let x = <Bn254 as Pairing>::ScalarField::rand(&mut OsRng);
        let crs = RawCRSElements::<Bn254> {
            alpha_1: <Bn254 as Pairing>::G1::generator() * alpha,
            beta_1: <Bn254 as Pairing>::G1::generator() * beta,
            beta_2: <Bn254 as Pairing>::G2::generator() * beta,
            x_1: vec![
                <Bn254 as Pairing>::G1::generator(),
                <Bn254 as Pairing>::G1::generator() * x,
                <Bn254 as Pairing>::G1::generator() * (x * x),
                // The important part
                <Bn254 as Pairing>::G1::generator() * (x * x),
            ],
            x_2: vec![
                <Bn254 as Pairing>::G2::generator(),
                <Bn254 as Pairing>::G2::generator() * x,
            ],
            alpha_x_1: vec![
                <Bn254 as Pairing>::G1::generator() * alpha,
                <Bn254 as Pairing>::G1::generator() * (alpha * x),
            ],
            beta_x_1: vec![
                <Bn254 as Pairing>::G1::generator() * beta,
                <Bn254 as Pairing>::G1::generator() * (beta * x),
            ],
        };
        assert!(crs.validate().is_err());
    }
    ...
    fn test_contribution_is_not_linked_if_degree_changes() {
        ...
        let root0 = CRSElements::<Bn254>::root(D);
        let root1 = CRSElements::<Bn254>::root(D + 1);
        let contribution = Contribution::make(
            &mut OsRng,
            ContributionHash([0u8; CONTRIBUTION_HASH_SIZE]),
            &root0,
