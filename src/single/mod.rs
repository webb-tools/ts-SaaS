...
pub struct ExtraTransitionInformation<E: Pairing> {
...
pub fn circuit_degree<E: Pairing>(circuit: &ConstraintMatrices<E::ScalarField>) -> Result<usize> {
    let circuit_size = circuit.num_constraints + circuit.num_instance_variables;
    Radix2EvaluationDomain::<E::ScalarField>::compute_size_of_domain(circuit_size)
...
pub fn transition<E: Pairing>(
    phase1: &phase1::CRSElements<E>,
    circuit: &ConstraintMatrices<E::ScalarField>,
) -> Result<(ExtraTransitionInformation<E>, phase2::CRSElements<E>)> {
    ...
    let circuit_size = circuit.num_constraints + circuit.num_instance_variables;
    let domain: Radix2EvaluationDomain<E::ScalarField> = Radix2EvaluationDomain::new(circuit_size)
        .ok_or_else(|| {
            anyhow!(
    ...
    let mut p_1 = vec![E::G1::zero(); qap_num_variables + 1];
    // Also take this opportunity to accumulate the raw u_1, v_1, and v_2 polynomials
    let mut u_1 = vec![E::G1::zero(); qap_num_variables + 1];
    let mut v_1 = vec![E::G1::zero(); qap_num_variables + 1];
    let mut v_2 = vec![E::G2::zero(); qap_num_variables + 1];
...
pub fn combine<E: Pairing>(
    circuit: &ConstraintMatrices<E::ScalarField>,
    phase1out: &Phase1CRSElements<E>,
    phase2out: &Phase2CRSElements<E>,
    extra: &ExtraTransitionInformation<E>,
) -> ProvingKey<E> {
    let vk = VerifyingKey {
        alpha_g1: phase1out.raw.alpha_1.into_affine(),
        beta_g2: phase1out.raw.beta_2.into_affine(),
        gamma_g2: E::G2::generator().into_affine(),
        delta_g2: phase2out.raw.delta_2.into_affine(),
        gamma_abc_g1: E::G1::normalize_batch(&extra.p_1[..circuit.num_instance_variables]),
    };
    ProvingKey {
        vk,
        beta_g1: phase1out.raw.beta_1.into_affine(),
        delta_g1: phase2out.raw.delta_1.into_affine(),
        a_query: E::G1::normalize_batch(&extra.u_1),
        b_g1_query: E::G1::normalize_batch(&extra.v_1),
        b_g2_query: E::G2::normalize_batch(&extra.v_2),
        h_query: E::G1::normalize_batch(&phase2out.raw.inv_delta_t_1),
        l_query: E::G1::normalize_batch(
...
mod test {
    ...
    struct TestCircuit<E: Pairing> {
        x: E::ScalarField,
        y: E::ScalarField,
        pub x_plus_y: E::ScalarField,
    }
    ...
    fn test_can_generate_keys_through_ceremony() -> anyhow::Result<()> {
        ...
        let degree = circuit_degree::<Bn254>(&matrices)?;
        ...
        let (extra, phase2root) = transition::<Bn254>(&phase1contribution.new_elements, &matrices)?;
        ...
        let ok = Groth16::<Bn254, LibsnarkReduction>::verify(&pk.vk, &public_inputs, &proof)?;
        assert!(ok);
